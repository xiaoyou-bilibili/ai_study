# 深度学习笔记

> 本项目主要记录了个人的一些练丹心得以及使用AI在各个领域的一些探索

# 练丹心得
- [linux环境安装](environment/linux_cuda.md)
- [windows环境安装](environment/windows_cuda.md)
- [conda安装和常用命令](tool/conda.md)
- [jupyter-lab安装](tool/jupyter.md)
- [docker显卡支持](environment/docker.md)
- [租显卡进行训练](environment/cloud.md)
- [使用Tmux进行后台训练](./command/tmux.md)
- [自动生成项目依赖的requirements.txt](./environment/save.md)


# AI项目实践

## 分类
### 卷积神经网络 CNN
- [CNN简单介绍](https://github.com/xiaoyou-bilibili/deep_cnn_Introduction)
### 人脸识别
- [动漫人脸识别](https://github.com/xiaoyou-bilibili/anime_recognize)

## 图像分割&目标检测
### 目标检测
- [yolov5目标检测](https://github.com/xiaoyou-bilibili/yolov5_demo)
- [实时目标检测](https://github.com/xiaoyou-bilibili/pi_camera_yolo)

## 姿态估计
### 3D姿态估计
- [根据视频自动生成MMD动作文件](https://github.com/xiaoyou-bilibili/openpose-mmd)
### 人脸姿态估计
- [二次元人物模仿视频人脸](https://github.com/xiaoyou-bilibili/openpose-wifu)


## 生成模型
### 生成对抗网络（GAN）
- [动漫头像生成](https://github.com/xiaoyou-bilibili/anime_avatar_gen)
### 扩散模型（diffusion）
- [AI作画](https://github.com/xiaoyou-bilibili/ai_draw)


## 语音相关
### 声纹识别
- [游戏角色识别](https://github.com/xiaoyou-bilibili/voice_recognize)
### 语音合成
- [语音合成项目](https://github.com/xiaoyou-bilibili/tts_vits)
### 声音克隆
- [把你的声音转换为原神角色声音](https://github.com/xiaoyou-bilibili/voice_clone)

## 强化学习
### DQN
- [强化学习玩超级马里奥](https://github.com/xiaoyou-bilibili/gym_super_mario)
